...
func generateMaskingInfo() Mask {
	...
	math_rand.Seed(int64(binary.LittleEndian.Uint64(byteSlice[:])))
	...
	//This is the max number that can be multiplied to the mask and still not go over max
	// i.e. 8421504 * 255 < 32bit max
	...
	//Set the first index to compare to the value at the index we randomly selected
